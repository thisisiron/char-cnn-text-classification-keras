...
def load_dataset(model='improved', mode='train', valid_split=0.1, max_len=1014):
    ...
    if mode=='train':
        ...
        train_data, tokenizer = preprocess(data_df['text'], model, max_len)
        ...
        test_data, tokenizer = preprocess(data_df['text'], model, max_len)
        data_df['target'] = data_df['target'].apply(lambda x: decode_label(x))
